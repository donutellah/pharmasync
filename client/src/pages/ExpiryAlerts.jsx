...
export default function ExpiryAlerts() {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: 'Inter,sans-serif', background: '#F8F9FA' }}>
      <AdminSidebar activePage="expiry" />
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ height: 64, background: 'white', borderBottom: '1px solid #F1F5F9', display: 'flex', alignItems: 'center', padding: '0 32px', gap: 14, flexShrink: 0 }}>
          ...
          <div style={{ flex: 1 }} />
          {(() => { const user = JSON.parse(localStorage.getItem('user') || '{}'); const initials = (user.name || 'A').split(' ').map(w => w[0]).join('').toUpperCase().slice(0,2); return (
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '32px 40px', display: 'flex', flexDirection: 'column', gap: 28 }}>
        ...
        {/* Header */}
        <div>
          <h1 style={{ margin: 0, color: '#281716', fontSize: 30, fontFamily: 'Manrope,sans-serif', fontWeight: 800 }}>Expiry Alerts Dashboard</h1>
              ...
              return (
                <div key={p.id} style={{ display: 'grid', gridTemplateColumns: '100px 1fr 100px 60px 110px 90px 120px 100px', padding: '14px 24px', gap: 8, alignItems: 'center', borderTop: i === 0 ? 'none' : '1px solid rgba(229,189,185,.1)' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'center', gap: 4 }}>
                    {days < 0 && <button style={{ padding: '3px 8px', background: 'rgba(209,31,39,.08)', border: 'none', borderRadius: 4, cursor: 'pointer', color: '#D11F27', fontSize: 10, fontWeight: 700 }}>Pull</button>}
                    {days !== null && days >= 0 && days <= 30 && <button style={{ padding: '3px 8px', background: 'rgba(217,119,6,.08)', border: 'none', borderRadius: 4, cursor: 'pointer', color: '#D97706', fontSize: 10, fontWeight: 700 }}>Discount</button>}
                    {days !== null && days > 30 && days <= 90 && <button style={{ padding: '3px 8px', background: 'rgba(6,182,212,.08)', border: 'none', borderRadius: 4, cursor: 'pointer', color: '#0891B2', fontSize: 10, fontWeight: 700 }}>Monitor</button>}
                    {(days === null || days > 90) && <span style={{ color: '#94A3B8', fontSize: 11 }}>—</span>}
                  </div>
                </div>
              );
            })}
