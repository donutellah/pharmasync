...
export default function AIAssistant() {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: 'Inter, sans-serif', background: '#F8F9FA', overflow: 'hidden' }}>
      {/* Sidebar */}
      <div style={{
        width: 240, minWidth: 240, height: '100vh', background: '#0B1C30',
        display: 'flex', flexDirection: 'column', justifyContent: 'space-between',
        padding: '24px 0', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)',
      }}>
        {/* Logo */}
        <div style={{ padding: '0 24px 32px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <div style={{ color: 'white', fontSize: 18, fontFamily: 'Manrope, sans-serif', fontWeight: 700, lineHeight: '24px' }}>Carlmed Admin</div>
              <div style={{ color: '#94A3B8', fontSize: 9, textTransform: 'uppercase', letterSpacing: 1, lineHeight: '14px' }}>Clinical Curator<br />Portal</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ flex: 1, padding: '0 16px', display: 'flex', flexDirection: 'column', gap: 2 }}>
          {NAV_ITEMS.map(item => {
            const active = item.path === '/ai-assistant';
            return (
              <div
                key={item.path}
                ...
                style={{
                  ...
                  background: active ? '#233244' : 'transparent',
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {insightsData.products.filter(p => p.quantity < 10).slice(0, 10).map(p => (
                        <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid rgba(192,199,207,0.15)' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                            <span style={{ fontSize: 12, color: '#004C6E' }}>₱{Number(p.price).toFixed(2)}</span>
                            <span style={{
                ...
                <div style={{ background: 'white', borderRadius: 14, padding: 20, outline: '1px solid rgba(192,199,207,0.15)' }}>
                  ...
                  {insightsData.sales.slice(0, 8).map(s => (
                    <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid rgba(192,199,207,0.15)' }}>
                      ...
                      <span style={{ fontSize: 13, fontWeight: 700, color: '#006C49' }}>₱{Number(s.total_amount).toFixed(2)}</span>
                    </div>
                  ))}
                </div>
              </div>
